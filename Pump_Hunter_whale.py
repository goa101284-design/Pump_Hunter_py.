        ...
        async with session.get(url, timeout=10) as resp:
            ...
            if res.get("code") == 0:
                ...
                for item in data:
                    ...
                    # 🔧 ФИЛЬТР: отсекаем индексы, форекс и не-крипто инструменты BingX
        ...
        async with session.get(url, params=params, timeout=10) as resp:
            ...
            if code == 0:
                ...
                if data:
                    data.reverse()
                return data
            elif code == 109429: # 🔧 ЗАЩИТА ОТ RATE LIMIT: если лимит превышен, берем паузу
...
# =====================================================================
# 🔄 ГЛАВНЫЙ ЦИКЛ СКАНИРОВАНИЯ (БЕЗ ОШИБКИ 109429)
# =====================================================================
...
async def scan_market():
    ...
    logging.info("🚀 Сканер запущен. Режим защиты API активен!")
    last_full_scan_time = 0

    async with aiohttp.ClientSession() as session:
        await send_telegram_message("🤖 **Сканер перезапущен и защищен от блокировок API!**")
        ...
        while True:
            ...
            try:
                ...
                # 🔧 РЕГУЛИРОВКА СКОРОСТИ: батч 5 монет + задержка 0.25с снимут блокировку 109429
                batch_size = 5
                for i in range(0, total_symbols, batch_size):
                    ...
                    await asyncio.sleep(0.25) # Пауза 250 мс предотвращает спам API

                duration = round(time.time() - start_time, 2)
                logging.info(f"💓 [HEARTBEAT] Круг завершен за {duration}сек | Сканировано монет: {total_symbols} | Найдено сигналов: {signals_found}")
