...
def find_base_params(klines_1h, min_hours, max_hours, max_width):
    """Ищет базу накопления. 🔧 Убрана агрессивная защита от ложных флэтов."""
    closed = klines_1h[:-1]
    ...
    for w in range(max_hours, min_hours - 1, -2):
        ...
        # 🔧 Защита от ложных флэтов ОТКЛЮЧЕНА для лучшего детекта альтов
        # ranges = [(float(k[2]) - float(k[3])) / float(k[3]) * 100 for k in recent]
        # avg_range = np.mean(ranges)
        # max_range = max(ranges)
        # if max_range > max_width * 2.2 and avg_range > max_width * 0.9: continue
...
def evaluate_realtime_signal(klines_1h):
    ...
    # 🔧 Расширенный поиск базы (3 уровня)
    ...
    # ---------------------------------------------------------------------
    # 🎯 СЦЕНАРИЙ 1: 49–56 МИНУТА (Залив объёма ВНУТРИ БАЗЫ)
    # ---------------------------------------------------------------------
    if is_flat and (IDEAL_ENTRY_MINUTES[0] <= int(elapsed) <= IDEAL_ENTRY_MINUTES[1]):
        if not (change_pct >= 1.2 and rvol >= 1.3):
            if change_pct <= 2.5 and rvol >= 1.2 and close_h > ema20_val:
                return {
                    "type": "H1_BREAKOUT",
                    "score": 90,
                    "reasons": [
                        f"🎯 **ЗАЛИВ В БАЗЕ ({int(elapsed)}я мин):** RVOL x{round(rvol,1)}, цена ещё в накоплении (+{round(change_pct,2)}%)!",
    ...
    # ---------------------------------------------------------------------
    # 🚀 СЦЕНАРИЙ 2: ПЕРВАЯ ИМПУЛЬСНАЯ СВЕЧА ИЗ БАЗЫ (снижены пороги)
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # 📈 РЕЖИМ 3: ПЛАВНЫЙ НАРАСТАЮЩИЙ ПАМП
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # 💥 РЕЖИМ 4: ОБЫЧНЫЙ / АГРЕССИВНЫЙ ОБЪЕМНЫЙ ПАМП (Без базы)
    # ---------------------------------------------------------------------
...
async def scan_market():
    ...
    logging.info("🚀 Сканер H1 запущен (база до 12%)...")
    last_full_scan_time = 0

    async with aiohttp.ClientSession() as session:
        await send_telegram_message("🤖 **Сканер обновлён!**\n• База до 12%\n• Защита от ложных флэтов отключена\n• Импульс из базы от 0.6%")
