...
def cleanup_old_data():
    ...
    # Очистка кулдаунов сигналов (старше 24 часов)
    expired_signals = [k for k, v in last_signals.items() if now - v > 86400]
    for k in expired_signals:
        del last_signals[k]
        
    # Очистка истории OI, если по монете давно не было обновлений
...
def evaluate_accumulation_signal(klines_1h, klines_5m, is_oi_valid, oi_delta_pct):
    ...
    # 1. Позиция цены относительно боковика
    ...
    async with semaphore:
        try:
            ...
            if signal['score'] >= MIN_SCORE_THRESHOLD:
                ...
                ticker_symbol = symbol.replace('/USDT', 'USDT').replace(':USDT', '')
                reasons_str = "\n• " + "\n• ".join(signal['reasons'])
                msg = (
                    f"🎯 **BINGX: РАННИЙ СИГНАЛ (Score: {signal['score']}/100)**\n"
                    f"📌 **Монета:** `{ticker_symbol}`\n"
                    f"💵 **Цена:** `${signal['price']}`\n"
...
async def scanner_loop():
    ...
    # Точный фильтр: активные бессрочные USDT-фьючерсы (Swap)
    ...
    while True:
        try:
            cleanup_old_data()  # Очищаем память каждый круг
            ...
            if valid_symbols:
                ...
                # Батчи по 50 монет для предотвращения перегрузки памяти на Render
                BATCH_SIZE = 50
...
async def safe_scanner_runner():
    """Защитный цикл: если сканер упадет по критической ошибке, он автоматически перезапустится"""
    while True:
        try:
            await scanner_loop()
        except Exception as e:
            logging.exception(f"💥 Критическая ошибка сканера! Перезапуск через 10 сек... Ошибка: {e}")
...
async def main():
    ...
    try:
        ...
        # Корректно закрываем сессию биржи при остановке приложения
