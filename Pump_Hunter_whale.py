...
# Внешний URL вашего сервиса на Render (или возьмите из переменной окружения RENDER_EXTERNAL_URL)
...
async def scanner_loop():
    logging.info("Сканер запущен на бирже BingX Futures...")
...
async def health_check_handler(request):
    """Обработчик HTTP GET запроса для Render health check"""
    return web.Response(text="Bot is running 24/7", status=200)


async def self_ping_loop():
    """Фоновая задача: каждые 10 минут отправляет запрос сам на себя, предотвращая усыпление"""
    await asyncio.sleep(30)  # Небольшая задержка перед первым пингом
    if not RENDER_EXTERNAL_URL:
        logging.warning("RENDER_EXTERNAL_URL не задан. Авто-пинг отключен. Задайте переменную окружения на Render.")
        return
