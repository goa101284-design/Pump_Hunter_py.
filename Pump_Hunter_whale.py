...
def parse_kline(kline):
    """Разбор свечи BingX / Binance: [open_time, open, high, low, close, volume, ...]"""
    o = float(kline[1])
    h = float(kline[2])
    l = float(kline[3])
    c = float(kline[4])
...
def evaluate_realtime_signal(symbol, klines_1h, volume_24h_usdt=0.0):
    ...
    if is_flat and len(klines_1h) >= 9:
        ...
        # Фильтр манипуляций/выноса стопов
        ...
        # Окно подтверждения объема (строго с 3 по 5 минуту)
        is_valid_window = (3 <= int(elapsed) <= 5)

        # Жесткая комплексная проверка
        if (quiet_history and 
            candle_range <= 1.5 and 
            body_pct <= 0.6 and 
            change_pct >= 0.0 and          # Только зелёные/нейтральные свечи
            close_h >= ema20_val and       # Строго над EMA20
            rvol >= 1.6 and 
            no_manipulation and            # Без выносов
            is_valid_window):              # Строго 3-5 минута
...
def format_telegram_message(symbol, signal):
    ...
    clean_symbol = symbol.replace('USDT', '').replace('-', '').replace('_', '').replace('#', '').strip()
...
# -------------------------------------------------------------
# ИНТЕГРАЦИЯ С ВЕБ-СЕРВЕРОМ И RENDER HEALTH-CHECK
# -------------------------------------------------------------
async def handle_ping(request):
    """Ответ для Render Health Check"""
    return web.Response(text="Crypto Pulse Scanner is Active!", status=200)


async def scanner_loop():
    """Фоновый асинхронный цикл сканера"""
    logging.info("🚀 Фоновый сканер успешно запущен...")
    while True:
        try:
            # Здесь вызывается твоя функция сканирования монет
            # logging.info("Сканирование рынка...")
            pass
        except Exception as e:
            logging.error(f"Ошибка в сканере: {e}")

        # Пауза 5 секунд между кругами
...
async def main():
    ...
    # Считываем PORT из системных переменных Render
    ...
    # Запускаем фоновую задачу сканирования рынка
    asyncio.create_task(scanner_loop())

    # Бесконечный ждущий режим
