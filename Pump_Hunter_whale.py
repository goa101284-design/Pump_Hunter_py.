...
# ============================================================
# ПАМП-ХАНТЕР v10.1 FIXED — Production Ready Edition
# ============================================================
...
ACCUM_MIN_RATIO = float(os.environ.get("ACCUM_MIN_RATIO", "2.5"))  # Мин. отношение роста OI к цене
...
log = logging.getLogger("PUMP-HUNTER-v10-FIXED")
...
# ============================================================
# FETCHERS — OI (Bitget parsing fixed)
# ============================================================
...
async def fetch_bitget_oi(symbol):
    ...
    if isinstance(raw, dict):
        # FIX: openInterestList парсится правильно
...
def calc_intracandle_metrics(candles):
    ...
    # FIX: elapsed_real замеряется строго до нормализации
...
def calc_accumulation(base, kc_candles):
    """
    Исправленный модуль тихого накопления: проверяем дисбаланс (Ratio),
    когда OI вырос значительно сильнее, чем сместилась цена.
    """
...
async def send_status_message():
    ...
    msg = (
        f"📊 <b>Статус PUMP-HUNTER v10.1 Fixed</b>\n\n"
...
async def handle_telegram_update(update):
    ...
    if text.startswith("/start"):
        await send_tg("👋 <b>PUMP-HUNTER v10.1 Fixed запущен.</b> Используй /status для получения отчёта.")
...
async def index(req):
    return web.Response(
        text=(f"PUMP-HUNTER v10.1 Fixed | Uni: {len(UNIVERSE)} | "
...
async def start(app):
    ...
    await send_tg("🚀 <b>PUMP-HUNTER v10.1 Fixed успешно развернут и запущен!</b>")
