        ...
        async with session.get(url, timeout=10) as resp:
            ...
            if res.get("code") == 0:
                ...
                for item in data:
                    ...
                    # ФИЛЬТР: отсекаем контракты с префиксами количества (1000..., 1000000...)
...
async def get_klines(session, symbol: str, interval: str, limit: int = 80):
    ...
    # 🔧 ИСПРАВЛЕНИЕ: BingX V3 API строго требует тикер с дефисом (BTC-USDT)
    ...
    try:
        async with session.get(url, params=params, timeout=5) as resp:
            ...
            elif code in [109400, 109404, 80001]: # Невалидный контракт BingX
...
async def process_symbol(session, symbol, now_ts):
    ...
    # Авто-удаление невалидных тикеров
...
async def scan_market():
    ...
    logging.info("🚀 Сканер запущен. Оптимизированное подключение BingX V3!")
    last_full_scan_time = 0

    async with aiohttp.ClientSession() as session:
        await send_telegram_message("🤖 **Сканер перезапущен с верным тикер-форматом BingX!**")
