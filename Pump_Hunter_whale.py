...
def find_last_accumulation_before_breakout(klines_1h):
    """
    Ищет ПОСЛЕДНЮЮ валидную базу непосредственно перед текущим пробоем.
    Перебирает все размеры от 6 до 24 часов (без break).
    Возвращает первую (ближайшую к текущей свече) подходящую базу.
    """
...
def evaluate_accumulation_expansion(symbol, klines_1h, volume_24h_usdt=0.0):
    """
    Иерархия: CONFIRMED → STRONG_EARLY → EARLY
    """
...
async def scanner_loop():
    logging.info("🚀 Сканер Accumulation→Expansion v2.1 запущен")
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        while True:
            try:
                ...
                if errors:
                    logging.warning(f"Ошибок в тасках: {len(errors)}")
                
                elapsed = asyncio.get_event_loop().time() - start_time
                logging.info(f"✅ Скан за {elapsed:.1f}с | Сигналов: {signals_found} | В кэше: {len(sent_signals)}")
...
async def handle_ping(request):
    return web.Response(text="Accumulation→Expansion v2.1 Active", status=200)
