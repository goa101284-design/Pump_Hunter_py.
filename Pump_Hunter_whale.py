        ...
        async with session.get(url, timeout=10) as resp:
            ...
            if res.get("code") == 0:
                ...
                for item in data:
                    ...
                    # 🔧 ФИЛЬТР: отсекаем индексы, форекс и не-крипто тикеры
                    if symbol.startswith("NC") or "USD2" in symbol or "NATGAS" in symbol: continue
                    # 🔧 ФИЛЬТР: отсекаем мультипликаторные контракты (1000..., 1000000...)
        ...
        async with session.get(url, params=params, timeout=5) as resp:
            ...
            if code == 0:
                ...
                if data:
                    data.reverse()
                return data
            elif code == 109429: # Rate limit
                await asyncio.sleep(1.0)
            elif code in [109400, 109404, 80001]: # 🔧 Невалидный контракт BingX — помечаем на удаление
                logging.warning(f"⚠️ Пара {symbol} недоступна в API (Code {code}). Удаляем из списка...")
...
async def process_symbol(session, symbol, now_ts):
    ...
    # 🔧 Если BingX ответил 109400/109404 — исключаем тикер из отслеживания
...
async def scan_market():
    ...
    logging.info("🚀 Сканер запущен. Режим авто-очистки битых пар активен!")
    last_full_scan_time = 0

    async with aiohttp.ClientSession() as session:
        await send_telegram_message("🤖 **Сканер перезапущен с фильтрацией битых пар!**")
