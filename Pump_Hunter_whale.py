...
# ============================================================
# ПАМП-ХАНТЕР v10.2 — Low-RAM & Copy-Ticker Edition
# ============================================================
...
# ОБНОВЛЕНО: Фильтр цены от 0.001 до 1.0 USDT
...
MIN_PUMP_PCT = float(os.environ.get("MIN_PUMP_PCT", "17.5"))
...
# ОБНОВЛЕНО: Оптимизирован лимит запрашиваемых свечей для сохранения RAM
...
log = logging.getLogger("PUMP-HUNTER-v10.2")
...
async def fetch_bitget_oi(symbol):
    ...
    if isinstance(raw, dict):
        items = raw.get("openInterestList") or raw.get("list")
...
def calc_accumulation(base, kc_candles):
    ...
    idx_now = len(kc_candles) - 2
    idx_start = max(0, idx_now - ACCUM_WINDOW_HOURS)

    start_close = kc_candles[idx_start]["close"]
    now_close = kc_candles[idx_now]["close"]
    if start_close <= 0:
        return None
...
async def send_accumulation_alert(base, acc):
    # ОБНОВЛЕНО: Тикер Bybit обёрнут в <code> для мгновенного копирования по клику
...
async def send_status_message():
    ...
    msg = (
        f"📊 <b>Статус PUMP-HUNTER v10.2</b>\n\n"
        ...
        f"🚨 Триггеров: {STATS['pump_triggers']} (fast={STATS['pump_triggers_fast']}, slow={STATS['pump_triggers_slow']})\n"
        f"✅ Сигналов отправлено: {STATS['signals']} (fast={STATS['signals_fast']}, slow={STATS['signals_slow']})\n"
...
async def handle_telegram_update(update):
    ...
    if text.startswith("/start"):
        await send_tg("👋 <b>PUMP-HUNTER v10.2 запущен.</b> Используй /status для получения отчёта.")
...
async def process_symbol(base):
    ...
    if ACCUM_ENABLED and time.time() - ACCUM_LAST_SIGNAL.get(base, 0) >= ACCUM_COOLDOWN_SEC:
        ...
        if acc:
            ...
            log.info("⚠️ НАКОПЛЕНИЕ: %s | ΔЦена %.1f%%, Ratio=%.2f", base, acc["price_change_pct"], acc["oi_to_price_ratio"])
...
async def oi_sample_cycle():
    ...
    # ОБНОВЛЕНО: Семафор уменьшен до 5 для снижения нагрузки на RAM
    ...
    gc.collect()  # Очистка неиспользуемой памяти
...
async def refresh_universe():
    ...
    log.info("🌐 Юниверс обновлен: %d пар (Диапазон цен: $%s - $%s)", len(UNIVERSE), MIN_PRICE_USDT, MAX_PRICE_USDT)
    gc.collect()
...
async def scan_cycle():
    ...
    # ОБНОВЛЕНО: Семафор уменьшен до 5 для быстрой работы на базовом тарифе Render
    semaphore = asyncio.Semaphore(5)
...
async def index(req):
    return web.Response(
        text=(f"PUMP-HUNTER v10.2 | Uni: {len(UNIVERSE)} | "
              f"Triggers: {STATS['pump_triggers']} | Signals: {STATS['signals']} | "
              f"Accum Alerts: {STATS['accum_alerts']} | Scans: {STATS['scans']} | "
...
async def start(app):
    ...
    START_TIME = time.time()
    # ОБНОВЛЕНО: Лимит соединений уменьшен для снижения памяти
    SESSION = aiohttp.ClientSession(connector=aiohttp.TCPConnector(limit=25, ttl_dns_cache=300))
    ...
    await send_tg("🚀 <b>PUMP-HUNTER v10.2 успешно развернут и запущен!</b>")
